...
- Hosted layer: test mode. `npm run test-mode` starts the local Supabase, fakes of GitHub, Slack and Stripe and the app with seeded test data (an Agency organization with two customer workspaces, runs of every status, upgrade checks for the drift matrix, an acceptance, a GitHub check and Slack messages, and a Free organization), with one-click sign-in as the seeded people, mail in Mailpit and a "Test mode" banner on every page. It needs Docker and no production account; `--reset` seeds again from an empty database. `FLOWRETEST_TEST_MODE` is ignored unless the app and its database run on localhost.
- Hosted layer: the first paid plan of an organization starts with a 14-day free trial (`STRIPE_TRIAL_DAYS`, 0 turns it off). Checkout takes the card and charges it when the trial ends; the billing page shows the date and the amount of the first charge. One trial per organization.
- Hosted layer: the Data Processing Agreement in Polish (`/legal/dpa?lang=pl`, PDF copy with `?lang=pl`; the English text prevails), and notices of sub-processor changes at least 30 days ahead, shown on `/legal/subprocessors` and on the Data page and emailed once to each owner of an organization that accepted the DPA (`apps/web/scripts/subprocessor-notice.ts`).
...
- Hosted layer, audit of week 14 (ADR 0018): the legal texts describe what redaction keeps readable (numbers below one million, true/false) and the data really stored (branch, GitHub account); VAT wording and a live Stripe key that needs `STRIPE_AUTOMATIC_TAX` set; the Terms of Service are accepted when an organization is created, with a term and termination clause. DPA acceptances are written by the server only and keep the provider details and the draft flag for their PDF. Sub-processor notices: 30 full days, counted from the email, unchangeable once sent, to the owners of every organization, one email per address even with two senders at once, refused without a mail transport. The export reads every table in pages and ends with a line count; large organizations no longer break it or the Data page. A second trial from two Checkout tabs is cancelled. Expired invitations free their seat at once; the auth sign-in log is kept 30 days. `POST /api/runs` drains the body before a 401, so clients get the answer instead of a reset connection.
