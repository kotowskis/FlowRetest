...
writeFileSync(out, [`NEXT_PUBLIC_SUPABASE_URL=${values.API_URL}`, `NEXT_PUBLIC_SUPABASE_ANON_KEY=${values.ANON_KEY}`, `SUPABASE_SERVICE_ROLE_KEY=${values.SERVICE_ROLE_KEY}`, `APP_URL=${appUrl}`, `MAILPIT_URL=${values.MAILPIT_URL ?? ''}`, '# Lets owners accept the draft DPA locally and in CI (ADR 0015); production waits for LEGAL_FINAL.', 'LEGAL_ALLOW_DRAFT_ACCEPTANCE=true', ''].join('\n'));
