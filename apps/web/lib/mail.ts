...
export async function sendMail(message: MailMessage, env: MailEnv = process.env as MailEnv): Promise<MailResult> {
  const from = env.MAIL_FROM || DEFAULT_FROM;
  if (env.RESEND_API_KEY) {
    const r = await post('https://api.resend.com/emails', { authorization: `Bearer ${env.RESEND_API_KEY}` }, { from, to: [message.to], subject: message.subject, text: message.text, html: message.html });
    return { ok: r.ok, transport: 'resend', detail: r.detail };
  }
  if (env.MAILPIT_URL) {
    const sender = parseFrom(from);
    const r = await post(`${env.MAILPIT_URL.replace(/\/+$/, '')}/api/v1/send`, {}, { From: { Email: sender.email, Name: sender.name ?? '' }, To: [{ Email: message.to }], Subject: message.subject, Text: message.text, HTML: message.html });
