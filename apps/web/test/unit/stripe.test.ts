...
test('subscription state: the plan comes from the price lookup key, the period from the item', () => {
  ...
  assert.deepEqual(active, { plan: 'team', interval: 'month', status: 'active', currentPeriodEnd: new Date(1_800_000_000_000).toISOString(), cancelAtPeriodEnd: false, endedAt: null });
