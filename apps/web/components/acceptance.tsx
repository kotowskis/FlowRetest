  ...
  return (
    <form action={run} className="space-y-4">
      ...
      <fieldset className="space-y-2">
        <legend className="mb-2 text-sm font-medium">Cases</legend>
        {cases.map((c) => (
          <label key={c.caseId} className={`flex flex-wrap items-baseline gap-2 text-sm ${c.blocked ? 'text-muted' : ''}`}>
            <input type="checkbox" name="case" value={c.caseId} defaultChecked={!c.blocked && c.status === 'DIFF'} disabled={!!c.blocked} />
            <span className="font-mono">case {c.caseId}</span>
      ...
      <label className="flex flex-col gap-1 text-sm">
        ...
        <textarea name="message" maxLength={2000} rows={2} placeholder="Customer id moved to the new CRM field" className="rounded-md border border-line px-3 py-2 text-sm" />
      </label>
