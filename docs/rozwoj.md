...
- testy integracyjne bez Supabase, aplikacji albo atrap pomijają się lokalnie, a przy ustawionym `CI` kończą błędem (`mustRun` w `test/integration/helpers.ts`). Uruchomienie z własną kopią aplikacji obok serwera deweloperskiego: `next build`, potem `next start --port 3101` i atrapa na 55391 z nadpisanymi `APP_URL`, `GITHUB_APP_API_URL`, `GITHUB_APP_WEB_URL`, `SLACK_WEBHOOK_HOSTS` i `STRIPE_API_URL`, te same zmienne dla `npm run test:integration`.
