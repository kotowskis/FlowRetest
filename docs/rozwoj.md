...
Zmiana podprocesora (ADR 0016, 0018): plik JSON z `effectiveOn` (co najmniej 31 dni naprzód, bo 30 pełnych dni liczy się od dnia po ogłoszeniu), `summary` i `changes`, potem w `apps/web` `node scripts/subprocessor-notice.ts announce <plik>` i `send <id>` (najpierw z `--dry-run`); lokalnie maile idą do Mailpita. `send` odmawia, gdy do zmiany zostało mniej niż 30 dni. Przeciw bazie spoza localhost skrypt wymaga `RESEND_API_KEY` i adresu `https` w `APP_URL` i nie czyta wtedy `.env.local`. Po dacie zmiany trzeba poprawić `SUBPROCESSORS` w `lib/legal/documents.ts`. Polski PDF kopii DPA z testu: `DPA_PL_OUT=<plik.pdf> node --test test/unit/subprocessor-notices.test.ts`.

Testy obciążeniowe uploadu (ADR 0015): na zbudowanej aplikacji (`next build`, `next start`) `node scripts/load-upload.ts --app http://127.0.0.1:3101 --pid <pid aplikacji> --out wynik.json` w `apps/web`. Skrypt zakłada organizacje tymczasowe w lokalnej bazie i usuwa je na końcu, także po Ctrl+C; bazy ani aplikacji spoza localhost nie przyjmuje. Przebieg zawężają `--scenarios steady,burst,large,limit,badtoken` oraz `--duration` z `--concurrency 10,25,50`.
