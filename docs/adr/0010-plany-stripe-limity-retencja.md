...
| Organizacja subskrypcji | po kliencie Stripe utworzonym dla organizacji (klucz idempotencji `frt-customer-<org>`); plan tylko z `lookup_key` ceny, nigdy z metadanych | plan wynika z ceny, którą Stripe faktycznie pobiera; metadane zostają stare po zmianie planu, np. w portalu |
